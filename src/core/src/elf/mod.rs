//! ELF utilities and loaders.
//!
//! This module is the single place for ELF parsing and loading logic.
...
fn validate_header(header: Elf64Ehdr) -> Result<()> {
...
fn dynamic_file_range(data: &[u8], header: Elf64Ehdr) -> Result<Option<(usize, usize)>> {
    let phoff = header.e_phoff as usize;
    let phentsize = header.e_phentsize as usize;
    let phnum = header.e_phnum as usize;
...
fn vaddr_to_offset(data: &[u8], header: Elf64Ehdr, vaddr: u64) -> Result<usize> {
    let phoff = header.e_phoff as usize;
    let phentsize = header.e_phentsize as usize;
    let phnum = header.e_phnum as usize;
