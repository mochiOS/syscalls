...
use viewkit::platform::vga;

/// mochiOS VGA/フレームバッファバックエンド
///
/// - mochiOS: syscall 経由で VRAM をマップし、`flush()` で転送
/// - Linux host: `backend-mochios-vga-hosted` を有効にすると hosted-vga で模擬
        ...
        let fb_info = {
            ...
            if fb_info.is_none() {
                ...
                let height = std::env::var("VGA_HEIGHT")
                    ...
                    .parse::<u32>()
                    .unwrap_or(480);
                vga::host_init_framebuffer(width, height);
                fb_info = vga::get_info();
            }
            fb_info
        };

        #[cfg(not(feature = "backend-mochios-vga-hosted"))]
        let fb_info = vga::get_info();

        let fb_info = fb_info.ok_or_else(|| {
            BackendError::InitializationFailed("vga::get_info failed".to_string())
        })?;

        let fb_ptr = vga::map_framebuffer().ok_or_else(|| {
            BackendError::MemoryMapFailed("vga::map_framebuffer failed".to_string())
        })?;
